...
TEMP_IMAGE = 'tmp.bmp'
TEMP_FILE = 'txt_content/'
SUBFOLDER=''
PROCESSED_DOCS = 'processed_docs'
UNPROCESED_DOCS = 'unprocessed_docs'
...
class TesseractNotFound(Exception): #When tesseract is not found in the path
    pass
#Check if tesseract is in the path raise TesseractNotFound otherwise
def check_path():
...
def image_to_string(file,filepath,subfolder,lang=None, psm=None):
    global SUBFOLDER
    SUBFOLDER=subfolder
    txt_dir=TEMP_FILE+'/'+SUBFOLDER+'/'
    txtFile=TEMP_FILE+'/'+SUBFOLDER+'/'+os.path.splitext(file)[0]
    if not os.path.exists(txt_dir):
        os.makedirs(txt_dir)
    process_request(filepath, txtFile, lang, psm) #Process command
    f = open(txtFile + ".txt","r") #Open back the file
    txt = f.read()
    f.close()
    #shutil.move(filepath, PROCESSED_DOCS + '/'+SUBFOLDER+"/" + file)
